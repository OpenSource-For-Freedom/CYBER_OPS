...
def install_selinux_tools(status_gui):
    status_gui.update_status("Installing SELinux tools...")
...
def configure_selinux(status_gui):
    ...
    exec_command("setenforce", ["1"], status_gui)
    exec_command("sed", ["-i", "s/^SELINUX=.*/SELINUX=enforcing/", "/etc/selinux/config"], status_gui)
...
def run_aideinit(status_gui):
    try:
        ...
        status_gui.update_status("Command timed out